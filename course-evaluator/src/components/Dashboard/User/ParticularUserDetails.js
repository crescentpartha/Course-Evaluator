...
import useFindParticularUser from '../../../hooks/useFindParticularUser';

const ParticularUserDetails = () => {
    const { userDetailsId } = useParams();
    const [user] = useFindParticularUser(userDetailsId);
    // console.log(user);
    const { degree, department, email, image, name, programme, credit, gender, designation, registration_no, role, semester, session, type, usn } = user;
    return (
        <div>
            <h2 className="text-3xl font-medium font-lobster mb-2 text-accent text-center uppercase">{role}'s Profile</h2>
                            ...
                            <div className='text-start'>
                                ...
                                <p className='font-semibold'>Email: <span className='font-normal italic'>{email !== null ? email : "None"}</span></p>
                                {
                                    (typeof designation === 'undefined')
                                        ? <p className='font-semibold'>Designation: <span className='font-normal italic'>None</span></p>
                                        : <p className='font-semibold'>Designation: <span className='font-normal italic'>{designation !== null ? designation : "None"}</span></p>
                                }
                                <p className='font-semibold'>Role: <span className='font-normal italic'>{role !== null ? role : "None"}</span></p>
                                {
...
export default ParticularUserDetails;